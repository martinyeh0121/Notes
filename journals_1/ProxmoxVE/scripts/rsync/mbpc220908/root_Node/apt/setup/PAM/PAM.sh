...
echo "==== 建立密碼規則登入提示（/etc/profile.d/password_policy.sh） ===="

cat << 'EOF' > /etc/profile.d/password_policy.sh
#!/bin/bash

if [[ $- == *i* ]]; then
    echo -e "\e[1;33m⚠️ 密碼設定規則如下:\e[0m"
    echo "  - 最小長度：12 個字元"
    echo "  - 至少包含："
    echo "    - 1 個大寫字母"
    echo "    - 1 個小寫字母"
    ...
    echo "  - 不可為回文（如 abccba）"
    echo "  - 不可與帳號、舊密碼過於相似"
    echo "  - 密碼有效期：90 天，最短使用期限：1 天，過期前 7 天提醒"
fi
EOF
