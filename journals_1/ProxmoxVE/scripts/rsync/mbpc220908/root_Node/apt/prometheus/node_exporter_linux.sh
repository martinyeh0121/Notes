...
echo " - systemd 服務檔 /etc/systemd/system/node_exporter.service 會被建立 / 覆蓋"
echo " - 若已有 node_exporter 在執行，也可能會導致啟動失敗"

# 檢查是否以 root 執行腳本
# $EUID 是當前執行者的 UID，root 的 UID 是 0
if [[ $EUID -ne 0 ]]; then
  echo "錯誤：請使用 sudo 執行此腳本，例如：sudo ./node_exporter.sh"
  exit 1
fi
...
if ss -tuln | grep -q ':9100'; then
  echo "⚠️  警告：port 9100 已被佔用，可能已有 node_exporter 或其他服務在執行。"
...
# 建立 node_exporter 使用者，無家目錄且不能登入
...
# 設定 node_exporter 服務於開機自動啟動
systemctl enable node_exporter

# 顯示成功訊息
echo "✅ Node Exporter 安裝並啟動成功！ 可用 systemctl status node_exporter 檢查"
